<script>
	...
	export let favouriteAnime;
	let favouriteAnimePageNo = 1;

	const loadFavouriteAnime = async () => {
		favouriteAnimePageNo++;

		const res = await fetchPaginatedAnimeData("FAVOURITES_DESC", favouriteAnimePageNo);
		favouriteAnime = favouriteAnime.concat(res);
	};
</script>

<div class="tab-panel">
	<div class="card-display">
		{#each favouriteAnime as anime, i (i)}
			<div class="card">
	...
	<button class="load-more-button" on:click={loadFavouriteAnime}>Load more</button>
</div>
